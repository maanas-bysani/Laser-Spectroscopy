...
def perform_gaussian_fits(x, y, bg_x, bg_y, initial_params_list):
    ...
    for params in initial_params_list:
        ...
        fit_bg, _ = op.curve_fit(gaussian, bg_x, bg_y, params, maxfev=1000000)
...
# subtracted_df, fp_df, arr = data_cleaning(plot=True)
# subtracted_df_all, fp_df_all, arr_all = data_cleaning(plot=False)
# np.savetxt("residue_all.txt", arr_all)
# subtracted_df_all.to_csv('subtracted_df_all.csv', index=False)
# fp_df_all.to_csv('fp_df_all.csv', index=False)
# print(subtracted_df_all)
# print(fp_df_all)

# subtracted_df = pd.read_csv("subtracted_df_all.csv")
# fp_df = pd.read_csv("fp_df_all.csv")
# print(subtracted_df)
# print(fp_df)
# print(subtracted_df_all.isna().sum())
# print(arr)

subtracted_df = pd.read_csv("subtracted_df_all.csv")
fp_df = pd.read_csv("fp_df_all.csv")
angles = np.arange(50, 251, 5)
# angles = np.arange(180, 201, 5)
# angles = [185]

def fp_analysis(angles = angles, subtracted_df = subtracted_df, fp_df = fp_df, plots = False, block = False):
    region_dfs_dict = {}
    region_params_dfs_dict = {}
    region_peak_params_dfs_dict = {} 
    ...
    for angle in angles:
        ...
        plt.plot(time, fp_data_scaled, label = f'Scaled Fabry Perot Spectrum (using angle {fp_angle})')
        ...
        peak_height_guess = 0.0006
        peak_separation_guess = 4000
        ...
        region_dfs = [region1_df]
        region_params_dfs = [region1_param_df]
        region_peak_params_dfs = [region1_peak_params_df]
        ...
        for i, (start, end) in enumerate(regions, start = 1):
            ...
            print(len(region_time))
            print(region_time)

            print(len(region_fp_signal))
            print(region_fp_signal)

            print(len(region_signal))
            print(region_signal)
            ...
            print("---")
            print("region_peak_params_dfs", region_peak_params_dfs)
            print("---")
            ...
            for idx, peak_idx in enumerate(peaks):
                ...
                start = max(peak_idx - window_size, region_fp_signal.index[0])
                end = min(peak_idx + window_size, len(region_fp_signal))
                print(start, end)
                ...
                popt, cov = op.curve_fit(lorentzian_with_offset, region_x, region_y, p0=initial_guess, maxfev=1000000)
        ...
        region_dfs_dict[f'{angle}'] = region_dfs
        region_params_dfs_dict[f'{angle}'] = region_params_dfs
        region_peak_params_dfs_dict[f'{angle}'] = region_peak_params_dfs
    ...
    return region_dfs_dict, region_params_dfs_dict, region_peak_params_dfs_dict, \
            time_conversion_factor_df, index_conversion_factor_df, \
                time_conversion_factor_list, index_conversion_factor_list


# region_dfs_dict, region_params_dfs_dict, region_peak_params_dfs_dict, \
#             time_conversion_factor_df, index_conversion_factor_df, \
#                 time_conversion_factor_list, index_conversion_factor_list = fp_analysis(plots = True, block = False)

# print(region_dfs_dict)
# print(region_params_dfs_dict)
# print(region_peak_params_dfs_dict)

# for name, df_list in region_dfs_dict.items():
#     df = df_list[0]
#     df.to_csv(f"save_data\signals\{name}.csv", index=False)

# for name, df_list in region_params_dfs_dict.items():
#     df = df_list[0]
#     df.to_csv(f"save_data\\fit_params\{name}.csv", index=False)

# for name, df_list in region_peak_params_dfs_dict.items():
#     df = df_list[0]
#     df.to_csv(f"save_data\peak_params\{name}.csv", index=False)

# print(time_conversion_factor_df)
# time_conversion_factor_df.to_csv('time_conversion_factor_df.csv', index=False)

# print(index_conversion_factor_df)
# index_conversion_factor_df.to_csv('index_conversion_factor_df.csv', index=False)

# print(time_conversion_factor_list)
# print(index_conversion_factor_list)


#### need to load in data, crop to desired peak, fit lorentzian, analyse fwhm trend and then done!
